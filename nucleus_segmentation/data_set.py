...
class DataSet():
    ...
    def generate_input_image(self,file,mode=""):
        ...
        if mode == "bf":
            ...
            # brightfield image  H, W, C= H, W , 2
            return image[:,:,1:3]
        elif mode == "pc":
            image = skio.imread(file_path)
            # phaste contrast image  H, W, C = H, W, 1
    ...
    def get_input_images_as_array(self, split_dir, mode):
        ...
        for line in lines:
            split = line.strip()
            image_path_brightfield = split+"_ch5.tiff"
            image_array_brightfield = self.generate_input_image(image_path_brightfield,mode="bf")

            image_path_phase_contrast = split+"_ch1.tiff"
            image_array_phast_contrast = self.generate_input_image(image_path_phase_contrast,mode="pc")
            
            combined_array = np.concatenate((image_array_brightfield, image_array_phast_contrast), axis=2)
    ...
    def create_torch_data_loader(self,x_train=None,y_train=None,x_val=None,y_val=None, x_test=None,y_test=None, batch_size=1,height=224,width=224):
        ...
        print(f"(UNCROPPED) - Number of images: {len(train_dataset)} - Dimension input image: {train_dataset[0][0].shape} - Dimension output image: {train_dataset[0][1].shape}")
